...
print(paste0("Number of genes in data set: ", nrow(dataSet)))
# # Pre-filtering genes with low total counts
#
# smallestGroupSize <- 2
# keep <- rowSums(counts(dataSet) >= 100) >= smallestGroupSize
# dataSet <- dataSet[keep,]
# print(paste0("Number of genes in data set after filtering: ", nrow(dataSet)))
...
for(control in 1:length(controls)){
  for(condition in 1:length(conditions)){
    ...
    df_path = file.path(comp_dir2,
                        paste0("results_NA_remove_", comparisons, ".csv"))
    df_path = gsub(work_dir, "./", df_path)
    
    write.csv(filter(results_df, !is.na(log2FoldChange)),
              file = df_path, quote = FALSE)

    df_path = file.path(comp_dir2,
                        paste0("PcB_targets_NA_remove_", comparisons, ".csv"))
    df_path = gsub(work_dir, "./", df_path)
    
    write.csv(filter(results_PcB, !is.na(log2FoldChange)),
              file = df_path, quote = FALSE)
...
#     # Export volcano plot
#     vp_path = file.path(comp_dir,
#                         paste0("volcano_PcB_lfcShrink_", comparisons, ".pdf"))
#     vp_path = gsub(work_dir, "./", vp_path)
#     
#     pdf(file = vp_path, width = 6.5, height = 5)
#     print(vp)
...
#     # Export volcano plot for all genes
#     vp_path = file.path(comp_dir,
#                         paste0("volcano_lfcShrink", comparisons, ".pdf"))
#     vp_path = gsub(work_dir, "./", vp_path)
#     
#     pdf(file = vp_path, width = 6.5, height = 5)
#     print(vp2)
#     dev.off()
# 
#     # Export distribution of p-values
#     vp_path = file.path(comp_dir,
#                         paste0("pvalues", comparisons, ".pdf"))
#     vp_path = gsub(work_dir, "./", vp_path)
#     
#     pdf(file = vp_path, width = 6.5, height = 5)
#     print(hist(results_df$pvalue, main=comparisons, xlab = "P-values"))
#     dev.off()
#     
#     # Export data frames with results
#     df_path = file.path(comp_dir2,
#                         paste0("results_lfcShrink_", comparisons, ".csv"))
#     df_path = gsub(work_dir, "./", df_path)
#     
...
# 
#     df_path = file.path(comp_dir2,
#                         paste0("PcB_lfcShrink_", comparisons, ".csv"))
#     df_path = gsub(work_dir, "./", df_path)
#     
#     write.csv(results_PcB, file = df_path, quote = FALSE)
# 
#     df_path = file.path(comp_dir2,
#                         paste0("results_lfcShrink_NA_remove_", comparisons, ".csv"))
#     df_path = gsub(work_dir, "./", df_path)
#     
#     write.csv(filter(results_df, !is.na(log2FoldChange)), 
#               file = df_path, 
...
# 
#     df_path = file.path(comp_dir2,
#                         paste0("PcB_lfcShrink_NA_remove", comparisons, ".csv"))
#     df_path = gsub(work_dir, "./", df_path)
#     
#     write.csv(filter(results_PcB, !is.na(log2FoldChange)), 
#               file = df_path, quote = FALSE)
...
# rlogCounts_mat <- rlogCounts_mat %>% na.exclude()
# # Plot heatmap
...
for(i in 1:length(targets_list)){
  csv1 = file.path(targets_list[i], paste0("PcB_targets_NA_remove_", 
...
for(i in 1:length(targets)){
  ...
  csv1 = file.path(f1, paste0("PcB_targets_NA_remove_", basename(f1), ".csv")) # K27M KO
  csv2 = file.path(f2, paste0("PcB_targets_NA_remove_", basename(f2), ".csv")) # K27M+
  ...
  qqp <- ggplot(tab_merge, aes(x = log2FoldChange.x,
    ...
    #            shape = 21) +
    geom_point(data = filter(tab_merge, log2FoldChange.x - log2FoldChange.y >= 0.585),
               size = 2, fill = "blue", color = "blue", shape = 21) +
    geom_point(data = filter(tab_merge, log2FoldChange.x - log2FoldChange.y <= -0.585),
               size = 2, fill = "red", color = "red", shape = 21) +
    # Add reference lines
    geom_abline(intercept = 0, slope = 1, color = "black") +
    geom_abline(intercept = 0.585, slope = 1, linetype = "dashed", color = "red") +
    geom_abline(intercept = -0.585, slope = 1, linetype = "dashed", color = "blue") +
...
for(i in 1:length(targets)){
  ...
  csv1 = file.path(f1, paste0("PcB_targets_NA_remove_", basename(f1), ".csv"))

  tab1 = read.csv(csv1)[,-1]
  
  csv2 = file.path(f1, paste0("results_NA_remove_", basename(f1), ".csv"))
  ...
  vp2 = makeVolcanoPlot(tab2,
                       ...
                       y_axis=tab2$pvalue,
                       nbreaks = 5,
                       nudgex = ifelse(abs(tab2$log2FoldChange) < 1.5, 
                                       -5 * tab2$log2FoldChange, 
                                       tab2$log2FoldChange),
                       nudgey = ifelse(-log10(tab2$pvalue) > 1 & -log10(tab2$pvalue) < 2, 
                                       1.5, 
                                       0),
                       maxoverlaps = 8
                       )
...
### lfcShrink volcano plots
```{r Make scaled volcano plot with lfcShrink}
# condition = "K27M+"
# ref_targets = c("CBX4.1", "CBX4.2")
# 
# targets = c("CBX4.1", "CBX4.2", "CBX2.1", "CBX2.3", "CBX8.1", "CBX8.2")
...
# f2 = list_dir[grepl(t2, list_dir)]
# 
# csv1 = file.path(f1, paste0("PcB_lfcShrink_", basename(f1), ".csv"))
# csv2 = file.path(f2, paste0("PcB_lfcShrink_", basename(f2), ".csv"))
# 
# # Load results tables for reference targets
# tab1 = read.csv(csv1)[,-1]
# tab2 = read.csv(csv2)[,-1]
...
#   f1 = list_dir[grepl(t_gsub, list_dir)]
# 
#   csv1 = file.path(f1, paste0("PcB_lfcShrink_NA_remove", basename(f1), ".csv"))
# 
#   tab1 = read.csv(csv1)[,-1]
#   
#   csv2 = file.path(f1, paste0("results_lfcShrink_NA_remove_", basename(f1), ".csv"))
# 
#   tab2 = read.csv(csv2)[,-1]
#   
#   # For PcB-only plot
...
#   # Export volcano plot
#   vp_path = file.path(comp_dir,
#                       paste0("volcano_scaled_PcB_lfcShrink", t, ".pdf"))
#   vp_path = gsub(work_dir, "./", vp_path)
#     
#   pdf(file = vp_path, width = 6.5, height = 5)
#   print(vp)
...
#   # Export volcano plot
#   vp_path = file.path(comp_dir,
#                       paste0("volcano_scaled_lfcShrink_", t, ".pdf"))
#   vp_path = gsub(work_dir, "./", vp_path)
#     
#   pdf(file = vp_path, width = 6.5, height = 5)
#   print(vp2)
...
for(i in 1:length(targets)){
  for(j in 1:length(targets2)){
    ...
    # print(f1) #
    # print(f2) #
    csv1 = file.path(f1, paste0("PcB_targets_NA_remove_", basename(f1), ".csv")) # K27M+
    csv2 = file.path(f2, paste0("PcB_targets_NA_remove_", basename(f2), ".csv")) # K27M+
    ...
    tab_merge = merge(tab1, tab2, by = "Gene")
    tab_merge = filter(tab_merge, pvalue.y <= 0.01)
    ...
    # tab_merge_label = filter(tab_merge, Gene %in% PcB_gene_names$Gene.Name)
    tab_merge_label = filter(tab_merge, Gene %in% common_genes$Gene)
    tab_merge_label = filter(tab_merge_label, log2FoldChange.x - log2FoldChange.y <= -0.585)
    ...
    qqp <- ggplot(tab_merge, aes(x = log2FoldChange.x,
      ...
      # Add reference lines
      # geom_abline(intercept = 0, slope = 1, color = "black") +
      geom_abline(intercept = 0.585, slope = 1, linetype = "dashed") +
      # geom_abline(intercept = -0.585, slope = 1, linetype = "dashed") +
      labs(x = paste0("Log2 Fold-Change (", comp, ")"),
           y = paste0("Log2 Fold-Change (", comp2, ")")) +
      scale_x_continuous(limits = c(-limits[1],-limits[2])) +
      ...
      geom_text_repel(data = tab_merge_label, aes(label = Gene),
                      ...
                      nudge_x = ifelse((tab_merge_label$log2FoldChange.x <= 0.5),
                                       -0.5,
                                       0.5),
                      max.overlaps = 10
      ) +
      theme_classic() +
      ggtitle(paste0("PcB targests\n", comp2)) +
...
### Prepare data for Q-Q plot comparing different KOs - upregulated  

    
```{r Prepare data for Q-Q plot comparing different KOs - upregulated}
# Pattern matching for each condition
condition1 = "K27M+"
condition2 = "K27M+"
targets2 = c("CBX4.1", "CBX4.2")
...
for(i in 1:length(targets)){
  for(j in 1:length(targets2)){
    t = paste0(targets[i], "_", condition1, "_vs_LUC_", condition1) 
    t2 = paste0(targets2[j], "_", condition2, "_vs_LUC_", condition2) # K27M+
    ...
    limits = c(min(range(tab_merge$log2FoldChange.x)), # K27M KO
               max(range(tab_merge$log2FoldChange.y))) #%>%  rev()
    ...
    condition_color2 <- "black"  # Default color if no pattern matches
    for (pattern in names(color_mapping)) {
      if (grepl(pattern, targets2[j])) {
        condition_color2 <- color_mapping[pattern]
        break
      }
    }
    ...
    qqp <- ggplot(tab_merge, aes(x = log2FoldChange.x,
      ...
      #            size = 2, fill = condition_color, color = condition_color, shape = 21) +
      geom_point(data = filter(tab_merge, log2FoldChange.x - log2FoldChange.y <= -0.585),
                 size = 2, fill = condition_color2, color = condition_color2, shape = 21) +
      # Add reference lines
      # geom_abline(intercept = 0, slope = 1, color = "black") +
      geom_abline(intercept = 0.585, slope = 1, linetype = "dashed", color = "black") +
      # geom_abline(intercept = -0.585, slope = 1, linetype = "dashed", color = "black") +
      ggtitle(paste0("Upregulated PcB targests\n", comp2)) +
      labs(x = paste0("Log2 Fold-Change (", comp, ")"),
           y = paste0("Log2 Fold-Change (", comp2, ")")) +
      scale_x_continuous(limits = c(0,limits[2])) +
      scale_y_continuous(limits = c(0,limits[2])) +
      geom_text_repel(data = tab_merge_label, aes(label = Gene),
                      size = 4.5,
                      # nudge_y = tab_merge$log2FoldChange.y *1.2,
                      nudge_y = ifelse((tab_merge_label$log2FoldChange.y <= 0.8), 
                                       -0.35, 
                                       0.35),
                      # nudge_x = ifelse((tab_merge_label$log2FoldChange.x <= 0.5), 
                      #                  -2, 
    ...
    # Export q-q plot
      qqp_path = file.path(comp_dir,
                          paste0("Q-Q-plot_upregulated.pdf"))
      qqp_path = gsub(work_dir, "./", qqp_path)
...
```{r Prepare data for Q-Q plot comparing different KOs - downregulated}
# Pattern matching for each condition
condition1 = "K27M+"
condition2 = "K27M+"
targets2 = c("CBX4.1", "CBX4.2")
...
for(i in 1:length(targets)){
  for(j in 1:length(targets2)){
    ...
    qqp <- ggplot(tab_merge, aes(x = log2FoldChange.x,
      ...
      # geom_abline(intercept = 0.585, slope = 1, linetype = "dashed", color = "black") +
      geom_abline(intercept = -0.585, slope = 1, linetype = "dashed", color = "black") +
      ggtitle(paste0("Downregulated PcB targests\n", comp2)) +
      labs(x = paste0("Log2 Fold-Change (", comp, ")"),
           y = paste0("Log2 Fold-Change (", comp2, ")")) +
      scale_x_continuous(limits = c(limits[1], 0)) +
      scale_y_continuous(limits = c(limits[1], 0)) +
      geom_text_repel(data = tab_merge_label, aes(label = Gene),
                      size = 4.5,
                      # nudge_y = tab_merge$log2FoldChange.y *1.2,
                      nudge_y = ifelse(( tab_merge_label$log2FoldChange.y >= -1), -0.5, -1),
                      # nudge_x = tab_merge$log2FoldChange.x / 1,
                      max.overlaps = 10
      ) +
      theme_classic() +
    ...
    # Export q-q plot
      qqp_path = file.path(comp_dir,
                          paste0("Q-Q-plot_downregulated.pdf"))
      qqp_path = gsub(work_dir, "./", qqp_path)
...
for(control in 1:length(controls)){
  for(condition in 1:length(conditions)){
    ...
    results_df = filter(results_df, !is.na(log2FoldChange))
